...
async function UUID() {
    return 'xxxxxxxx-xxxx-Firex-Sx-bot-xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function(c) {
...
async function onCallbackQuery(event, callbackQuery) {
    ...
    if (callbackData.startsWith("delete_")) {
        ...
        if (deleteResponse.ok) {
            // Notify the user
            await answerCallbackQuery(callbackQuery.id, "âœ… File deleted successfully!");
            // Delete the bot's response message
            await deleteMessage(chatId, messageId);
        } else {
            await answerCallbackQuery(callbackQuery.id, "âŒ Failed to delete file.");
        }
    }
}
